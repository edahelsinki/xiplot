...
Run `python3 -m xaiplot` at the root directory.

## Dataframe loading and saving

`xaiplot` uses `pandas` to load dataframes from `.csv`, `.json`, `.pkl`, and &ndash; if `feather-format` is available &ndash; `.ft` files. If you are running `xaiplot` locally, you can simply copy your datasets into the `data/` folder. If you are accessing `xaiplot` remotely or you do not want to pollute the `data/` folder, you can upload the dataset into memory directly inside `xaiplot`: navigate to the data tab and either click the upload button to the right or drag the file into it.
...
`xaiplot` also supports saving the plots you have created in combination with the data it was generated with. To download a `.tar` archive containing the original dataset, auxiliary columns generated by `xaiplot`, and the settings of all plots, navigate to the data tab and press the "Download the combined plots-and-data file" button.

You can also resume working on a previously saved plots-and-data file by uploading it, just like a dataframe as described above, using the upload button in the data tab. Once the data itself is loaded, all plots will be recreated. Note that if any plot recreation fails, you will receive an error message popup, but `xaiplot` will still attempt to recreate the other plots as well.

### The combined plots-and-data file format

If you download a combined plots-and-data file from `xaiplot`, you will receive a `.tar` file, whose format you must follow if you want to generate your own to plots-and-data file to upload.
...
The `meta.json` file contains metadata that describe the state of `xaiplot` upon saving, including the settings for all created plots. It follows the following (loose) json schema, where programmatic interpolations are marked by comments and `VARIABLES`:
...
and where `PLOT_TYPES` is the list of plot type names registered with `xaiplot`, with `PLOT_SETTINGS` being the additional properties defined for that plot type. At present, `xaiplot` defines the following plot types, which define their respective plot settings objects:
