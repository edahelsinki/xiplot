...
The dashapp uses `pandas` to load dataframes from `.csv`, `.json`, `.pkl`, and &ndash; if `feather-format` is available &ndash; `.ft` files. If you are running the dashapp locally, you can simply copy your datasets into the `data/` folder. If you are accessing the dashapp remotely or you do not want to pollute the `data/` folder, you can upload the dataset into memory directly inside the dashapp: navigate to the data tab and either click the upload button to the right or drag the file into it.
...
The dashapp also supports saving the plots you have created in combination with the data it was generated with. To download a `.tar` archive containing the original dataset, auxiliary columns generated by the dashapp, and the settings of all plots, navigate to the data tab and press the "Download the combined plots-and-data file" button.

You can also resume working on a previously saved plots-and-data file by uploading it, just like a dataframe as described above, using the upload button in the data tab. Once the data itself is loaded, all plots will be recreated. Note that if any plot recreation fails, you will receive an error message popup, but the dashapp will still attempt to recreate the other plots as well.

### The combined plots-and-data file format

If you download a combined plots-and-data file from the dashapp, you will receive a `.tar` file, whose format you must follow if you want to generate your own to plots-and-data file to upload.
...
The `meta.json` file contains metadata that describe the state of the dashapp upon saving, including the settings for all created plots. It follows the following (loose) json schema, where programmatic interpolations are marked by comments and `VARIABLES`:
...
and where `PLOT_TYPES` is the list of plot type names registered with the dashapp, with `PLOT_SETTINGS` being the additional properties defined for that plot type. At present, the dashapp defines the following plot types, which define their respective plot settings objects:
